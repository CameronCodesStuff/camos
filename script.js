...
var ntimer=null, modalCB=null;
var bootIdx=0;
var BOOT_STEPS=['BIOS v2.40 -- UEFI Firmware','Detecting CPU: 0x'+Math.floor(Math.random()*0xFFFF).toString(16).toUpperCase(),'Memory test: OK -- 16384MB','Initializing storage controller','Mounting virtual filesystem: /dev/vda1','Loading kernel modules [OK]','Starting network subsystem [OK]','Launching cam-wm 3.0 [OK]','Loading user environment [OK]','Starting desktop session [OK]'];
...
function bootStep() {
  ...
  if (log) {
    ...
    line.className = 'blog';
    line.innerHTML = '<span class="bp">[' + String(bootIdx).padStart(2,'0') + ']</span> <span class="bt">' + s + '</span>';
    log.appendChild(line);
    log.scrollTop = log.scrollHeight;
  }
  bootIdx++;
  setTimeout(bootStep, 200 + Math.random() * 200);
}
...
function initDesktopCanvas() {
  ...
  for (var i=0;i<30;i++) pts.push({x:Math.random()*c.width,y:Math.random()*c.height,vx:(Math.random()-0.5)*0.1,vy:(Math.random()-0.5)*0.1,r:Math.random()+0.2,a:Math.random()*0.15+0.04});
  function draw(){
...
function startDesktop() {
  ...
  setTimeout(function(){showNotif('welcome','Double-click icons to open apps.');}, 900);
}
...
var CMDS={
  help:function(){return 'ls  cd  pwd  cat  echo  mkdir  touch  rm  clear  date  whoami  uname  uptime  neofetch';},
...
function brDownload(url, filename) {
  ...
  function tryFetch(fetchUrl) {
    return fetch(fetchUrl).then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.blob();});
  }
  tryFetch(url)
    .catch(function(){return tryFetch(PROXIES[0](url));})
    .catch(function(){return tryFetch(PROXIES[1](url));})
    .then(function(blob){
...
function brRenderDlPanel(){
  ...
  downloads.slice().reverse().forEach(function(d){
    ...
    var icon=['mp3','wav','ogg','flac'].indexOf(ext)>-1?'ti-music':['mp4','webm','mkv'].indexOf(ext)>-1?'ti-video':['jpg','jpeg','png','gif','webp','svg'].indexOf(ext)>-1?'ti-photo':['zip','rar','7z'].indexOf(ext)>-1?'ti-archive':['pdf'].indexOf(ext)>-1?'ti-file-type-pdf':'ti-file';
...
function brShow(which){
  ...
  panels.forEach(function(p){var el=$br('br-'+p);if(el)el.classList.toggle('vis',p===which);});
...
async function brNavTo(url,skipHistory){
  ...
  for(var pi=0;pi<PROXIES.length;pi++){
    try{
      if(lmsg)lmsg.textContent='Proxy '+(pi+1)+'/'+PROXIES.length+'...';
      var resp=await fetch(PROXIES[pi](url),{signal:AbortSignal.timeout(9000)});
      if(!resp.ok)continue;
